...
const terminologyResolutionArtifact = {
  ...transcriptArtifact,
  digest: "b".repeat(64),
  artifact_kind: "terminology-resolution",
  schema: { id: "watchcraft.terminology-resolution", version: 1 },
...
const topicNormalizationSpec = {
  ...
  handler: { id: "watchcraft.normalize.collection-topics", version: "10" },
...
const terminologyResolutionSpec = {
  ...
  handler: { id: "watchcraft.resolve.collection-terminology", version: "7" },
  source: { media_asset_id: "catalog-project:essence-of-linear-algebra" },
  inputs: [],
  dependencies: [
    transcriptArtifact,
...
const terminologyCheckpointArtifact = {
  ...transcriptArtifact,
  digest: "6".repeat(64),
  artifact_kind: "terminology-resolution-checkpoint",
  schema: { id: "watchcraft.terminology-resolution-checkpoint", version: 1 },
  key: `objects/sha256/66/${"6".repeat(62)}`,
};
const collectionCompilationSpec = {
  ...
  handler: { id: "watchcraft.compile.video-collection", version: "7" },
  source: { media_asset_id: "catalog-project:essence-of-linear-algebra" },
...
test("the checked-in registry is valid, stable, and fully resolves an approved job", () => {
  ...
  assert.equal(spec.registry_snapshot?.registry_version, "2026-09-15.1");
...
test("terminology resolution binds the complete transcript and draft-analysis corpus", () => {
  ...
  assert.equal(spec.registry_snapshot?.handler.version, "7");
  assert.equal(spec.registry_snapshot?.execution_profile.id, "python-openai");
  assert.equal(spec.dependencies.length, 4);
  assert.equal(
    resolveJobSpecAgainstRegistry(
      { ...terminologyResolutionSpec, inputs: [terminologyCheckpointArtifact] },
      DEFAULT_CAPABILITY_REGISTRY,
    ).inputs[0]?.artifact_kind,
